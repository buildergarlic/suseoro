...
def _make_valid_empty_xlsb(path: Path) -> None:
    """Build a minimal BIFF12 workbook based on calamine's public reproducer."""
    ...
    workbook += record(
        0x009C,
        struct.pack("<II", 0, 0) + wide_string("rId1") + wide_string("Sheet1"),
    )
    workbook += record(0x0090) + record(0x0084)
    sheet = (
        record(0x0081)
        + record(0x0094, struct.pack("<IIII", 0, 0, 0, 0))
        + record(0x0091)
        + record(0x0092)
        + record(0x0082)
    )
...
def test_calamine_parses_real_spreadsheet_formats(
    ...
    result = parse_tabular(target, role=DocumentRole.PURCHASE_REQUEST)
...
def test_xlsb_signature_is_recognized_and_broken_payload_is_accounted_for(
    ...
    result = parse_tabular(target, role=DocumentRole.UNKNOWN)
...
def test_calamine_accepts_a_valid_minimal_xlsb(tmp_path: Path) -> None:
    """Treating every XLSB package as broken or routing it to openpyxl must fail."""
    target = tmp_path / "valid-empty.xlsb"
    _make_valid_empty_xlsb(target)
...
def test_csv_tsv_txt_and_paste_detect_encoding_delimiter_and_stream_rows(
    ...
    parsed = [
        parse_tabular(csv_path, role=DocumentRole.PURCHASE_REQUEST),
        parse_tabular(tsv_path, role=DocumentRole.PURCHASE_REQUEST),
        parse_tabular(txt_path, role=DocumentRole.PURCHASE_REQUEST),
        parse_pasted_text(
            "ISBN\t제목\t수량\n00999\t붙여넣기 책\t5\n",
...
def test_complex_xlsx_handles_notes_merge_sheets_blanks_duplicates_and_formulas(
    ...
    result = parse_tabular(target, role=DocumentRole.VENDOR_QUOTE)
    ...
    assert "FORMULA_NOT_EVALUATED" in {
