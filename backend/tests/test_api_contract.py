...
def _upload_durable_state(settings: Settings) -> dict[str, object]:
    tables = (
        "source_files",
        "source_documents",
        "workspace_sources",
    ...
    blobs = tuple(
        sorted(
            path.relative_to(settings.sources_dir).as_posix()
            for path in settings.sources_dir.rglob("*")
            if path.is_file()
        )
    )
    return {"counts": counts, "blobs": blobs}
...
def test_accepted_upload_retry_replays_exact_202_without_duplicate_mutation(
    data_dir: Path,
) -> None:
    """Reserving after source insertion makes the second accepted request fail 409."""
    client, settings, csrf = _client(data_dir, raise_server_exceptions=False)
    request_files = [
        ("files", ("accepted-replay.csv", b"title,author\nA,B\n", "text/csv"))
    ]

    with client:
        first = client.post(
            f"/api/v2/workspaces/{WORKSPACE_ID}/sources",
            headers=_headers(csrf, "accepted-upload-replay"),
            data={"role": "PURCHASE_REQUEST"},
            files=request_files,
        )
        replay = client.post(
    ...
    assert _upload_durable_state(settings) == {
        "counts": {
            "source_files": 1,
            "source_documents": 1,
            "workspace_sources": 1,
        ...
        "blobs": (
            "f8/e2/f8e2c5ee638e9081f7183690d5d8062f382128951aaeeb1408c21e51cf357b6a",
        ),
    }


def test_partial_upload_retry_replays_exact_207_items_without_duplicate_mutation(
    data_dir: Path,
) -> None:
    """A partial request must replay its accepted IDs and rejection details verbatim."""
    client, settings, csrf = _client(data_dir, raise_server_exceptions=False)
    request_files = [
        ("files", ("partial-replay.csv", b"title,author\nC,D\n", "text/csv")),
        (
            "files",
            ("partial-replay.exe", b"MZ-rejected", "application/octet-stream"),
        ),
    ]
    ...
    with client:
        first = client.post(
            f"/api/v2/workspaces/{WORKSPACE_ID}/sources",
            headers=_headers(csrf, "partial-upload-replay"),
            data={"role": "PURCHASE_REQUEST"},
            files=request_files,
        )
        replay = client.post(
    ...
    assert [first.status_code, replay.status_code] == [207, 207]
    assert replay.json() == first.json()
    assert [item["status"] for item in replay.json()["items"]] == [
        "ACCEPTED",
        "FAILED",
    ]
...
def test_upload_key_reuse_with_changed_accepted_bytes_conflicts_without_blob_leak(
    data_dir: Path,
) -> None:
    """Publishing a changed body before fingerprint conflict leaves an orphan blob."""
    client, settings, csrf = _client(data_dir, raise_server_exceptions=False)

    with client:
        first = client.post(
            f"/api/v2/workspaces/{WORKSPACE_ID}/sources",
            headers=_headers(csrf, "changed-accepted-upload"),
            data={"role": "PURCHASE_REQUEST"},
            files=[("files", ("changed.csv", b"title,author\nFirst,A\n", "text/csv"))],
        )
        before_conflict = _upload_durable_state(settings)
    ...
    assert conflict.json()["detail"] == {
        ...
        "message": "같은 중복 요청 방지 키가 다른 요청에 사용되었습니다.",
        "request_id": REQUEST_ID,
        "fields": [],
    }
...
def test_partial_upload_fingerprint_includes_changed_rejected_file_bytes(
    data_dir: Path,
) -> None:
    """Hashing only accepted files lets a different rejected body replay as identical."""
    client, settings, csrf = _client(data_dir, raise_server_exceptions=False)
    accepted = ("files", ("same.csv", b"title,author\nSame,A\n", "text/csv"))

    with client:
        first = client.post(
            f"/api/v2/workspaces/{WORKSPACE_ID}/sources",
            headers=_headers(csrf, "changed-rejected-upload"),
            data={"role": "PURCHASE_REQUEST"},
            files=[
                accepted,
                (
                    "files",
                    ("bad.exe", b"MZ-rejected-one", "application/octet-stream"),
                ),
            ],
        )
        before_conflict = _upload_durable_state(settings)
        conflict = client.post(
            ...
            files=[
                ...
                (
                    "files",
                    ("bad.exe", b"MZ-rejected-two", "application/octet-stream"),
                ),
            ],
        )
...
def test_concurrent_identical_uploads_commit_once_and_both_return_exact_202(
    ...
    """Two same-key requests must serialize at the claim instead of racing constraints."""
    client, settings, csrf = _client(data_dir, raise_server_exceptions=False)
    barrier = Barrier(2)
    ...
    def upload_once(_index: int):
        ...
        return client.post(
            f"/api/v2/workspaces/{WORKSPACE_ID}/sources",
            headers=_headers(csrf, "concurrent-identical-upload"),
            data={"role": "PURCHASE_REQUEST"},
            files=[
                (
                    "files",
                    ("concurrent.csv", b"title,author\nConcurrent,A\n", "text/csv"),
                )
            ],
        )
...
def test_adjacent_upload_integrity_conflict_never_exposes_sqlite_identifiers(
    ...
    """A real uniqueness failure must not disclose tables, columns, or constraint text."""
    client, settings, csrf = _client(data_dir, raise_server_exceptions=False)
    request_files = [
        ("files", ("duplicate.csv", b"title,author\nDuplicate,A\n", "text/csv"))
    ]

    with client:
        first = client.post(
            f"/api/v2/workspaces/{WORKSPACE_ID}/sources",
            headers=_headers(csrf, "duplicate-upload-first-key"),
            data={"role": "PURCHASE_REQUEST"},
            files=request_files,
        )
        before_conflict = _upload_durable_state(settings)
        conflict = client.post(
            f"/api/v2/workspaces/{WORKSPACE_ID}/sources",
            headers=_headers(csrf, "duplicate-upload-second-key"),
            data={"role": "PURCHASE_REQUEST"},
            files=request_files,
        )
    ...
    assert conflict.json()["detail"] == {
        ...
        "message": "요청이 현재 데이터와 충돌합니다.",
        "request_id": REQUEST_ID,
        "fields": [],
    }
    serialized = json.dumps(conflict.json(), ensure_ascii=False).casefold()
    for internal in (
        "sqlite",
        "unique constraint",
        "constraint failed",
