...
describe("후보 만들기 자료 입력", () => {
  ...
  test("교체 파일을 연달아 고르면 최신 세대만 남기고 비교는 한 번만 시작한다", async () => {
      ...
      uploadSources: async (_workspaceId, input) => {
        if (initial) {
          ...
          return {
            job_id: "ingest-ready",
            items: [
              { filename: "ready.csv", status: "ACCEPTED", source_id: "source-ready", procurement_import_id: null, error: null, repair_obligation_id: null, repair_generation: null },
              { filename: "broken.exe", status: "FAILED", source_id: null, procurement_import_id: null, error: { code: "UNSUPPORTED_FILE_TYPE", message: "지원하지 않는 파일 형식입니다." }, repair_obligation_id: "repair-o", repair_generation: 0 },
            ],
          };
        }
    ...
    act(() => pending.get(2)?.({
      job_id: "ingest-b",
      items: [{ filename: "replacement-b.csv", status: "ACCEPTED", source_id: "source-b", procurement_import_id: null, error: null, repair_obligation_id: "repair-o", repair_generation: 2 }],
    }));
    await waitFor(() => expect(comparisons).toEqual([["source-ready", "source-b"]]));
    act(() => pending.get(1)?.({
      job_id: "ingest-a",
      items: [{ filename: "replacement-a.csv", status: "ACCEPTED", source_id: "source-a", procurement_import_id: null, error: null, repair_obligation_id: "repair-o", repair_generation: 1 }],
    }));
    await act(async () => undefined);
