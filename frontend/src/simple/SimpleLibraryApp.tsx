...
export function SimpleLibraryApp({ api = defaultApi }: { api?: LibraryApi }) {
  ...
  return <div className={`simple-app density-${display.density}${menuOpen ? " menu-open" : ""}`}>
    ...
    <aside ref={menuPanel} className="sidebar" aria-label="작업 메뉴" role={menuOpen ? "dialog" : undefined} aria-modal={menuOpen || undefined}>
      ...
      <div className="sidebar-bottom"><DisplaySettings settings={display} disabled={!bootstrap} /><div className="local-note"><span className="status-dot" /><span>내 컴퓨터에 자동 저장</span></div><button className="settings-button" onClick={() => openDialog({ kind: "settings" })} disabled={!bootstrap}><CommandIcon name="settings" /> 학교 설정 · 백업</button><button className="help-button" onClick={() => openDialog({ kind: "help" })}><span aria-hidden="true">?</span> 사용설명서</button><button className="support-button" onClick={() => openDialog({ kind: "support" })}><span aria-hidden="true">♡</span> 개발자 후원</button><details className="cover-attribution"><summary>표지 정보 제공처</summary><a href="https://www.aladin.co.kr/" target="_blank" rel="noopener noreferrer">알라딘</a> · <a href="https://openlibrary.org/" target="_blank" rel="noopener noreferrer">Open Library</a> · <a href="https://books.google.com/" target="_blank" rel="noopener noreferrer">Google Books</a></details><span className="sidebar-version">수서로 v{bootstrap?.version || "2.2.2"}</span></div>
      </div>
    </aside>
