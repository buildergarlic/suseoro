...
export function SimpleLibraryApp({ api = defaultApi }: { api?: LibraryApi }) {
  ...
  function selectList(id: string) { if (id === listId) return; activeList.current = id; setListId(id); setDetail(null); setLoading(true); setSearch(""); setTab("all"); setPage(0); setNotice(""); setError(""); }
  ...
  return <div className={`simple-app density-${display.density}`}>
    ...
    {dialog?.kind === "import" && detail && <ImportDialog api={api} initialKind={dialog.importKind} listId={listId} onClose={closeDialog} onImported={async (message, operationId) => { if (operationId) bulk.remember(operationId, "추천목록 통합 가져오기"); await refresh(); setNotice(message); }} />}
