...
import { emptyBook, errorMessage, type Book, type BookFields } from "./types";

export function BookFieldsForm({ value, onChange }: { value: BookFields; onChange: (value: BookFields) => void }) {
  const set = <K extends keyof BookFields>(key: K, next: BookFields[K]) => onChange({ ...value, [key]: next });
  return <div className="book-form-grid">
