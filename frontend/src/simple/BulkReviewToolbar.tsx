...
export function BulkReviewToolbar({ bulk, visible, pageBooks, onRequestDelete }: { bulk: ReturnType<typeof useBulkReview>; visible: Book[]; pageBooks: Book[]; onRequestDelete: () => void }) {
  ...
  return <details className="bulk-review" onKeyDown={event => {
    ...
    <div className="bulk-review-controls">
      ...
      <button className="button danger small" disabled={blocked || !bulk.checked.size} onClick={onRequestDelete}>선택한 책 삭제</button>
      {bulk.busy && <span role="status">일괄 작업을 저장하고 있습니다…</span>}
    </div>
    <p className="field-help">원문과 서지를 확인한 책은 ISBN 없이도 확인 완료할 수 있습니다. 가격 누락·잘못된 ISBN·불완전한 서지는 남겨 둡니다. 삭제는 작업 체크한 책만 대상으로 하며, 구입 체크와는 별개입니다.</p>
